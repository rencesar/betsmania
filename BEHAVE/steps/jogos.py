...
def define_games_to_challenger(context):
...
def matchs_on_page(context):
    rows = context.browser.find_by_css('table tbody tr')
    assert_equals(len(rows), len(context.table))

    for row, data in zip(rows, context.table):
